...
def find_nearest_row(lat, lon, date_str=None):
    lat_col, lon_col, date_col = None, None, None
    for c in df.columns:
        if c.lower() in ("latitude","lat","lat_deg","y"): lat_col = c
        if c.lower() in ("longitude","lon","lon_deg","x"): lon_col = c
        if c.lower() in ("date","time","datetime") and date_col is None: date_col = c
...
def predict_from_location_endpoint():
    ...
    temp_col, wind_col = None, None
    for c in df.columns:
        if c.lower() in ("temperature_c","temperature"): temp_col = c
        if c.lower() in ("temperature_k",): temp_col = c
        if c.lower() in ("wind_speed_ms","wind_speed"): wind_col = c
...
def predict_next_days():
    ...
    try:
        nearest = find_nearest_row(lat, lon)
    except ValueError as e:
        return jsonify({"error": str(e)}), 500
